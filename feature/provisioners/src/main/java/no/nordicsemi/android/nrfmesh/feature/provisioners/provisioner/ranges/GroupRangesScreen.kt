...
@Composable
internal fun GroupRangesScreen(
    snackbarHostState: SnackbarHostState,
    groupRanges: List<GroupRange>,
    otherGroupRanges: List<GroupRange>,
    ...
    val ranges = remember {
        mutableStateListOf<Range>().apply { addAll(elements = groupRanges) }
    }
    val overlaps by remember {
        derivedStateOf { ranges.overlaps(other = otherGroupRanges) }
    }
    RangesScreen(
        title = stringResource(id = R.string.label_group_ranges),
        ranges = ranges,
        otherRanges = otherGroupRanges,
        overlaps = overlaps,
        addRange = { start, end ->
        ...
        resolve = {
            val list = ranges.minus(other = otherGroupRanges)
            ranges.clear()
            ranges.addAll(list)
        },
        save = {
