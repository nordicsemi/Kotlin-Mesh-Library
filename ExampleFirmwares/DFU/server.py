...
class MyHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Distributor nRF52840 1.0 -> 2.0
        elif path == '/check' and query.get('cfwid', [None])[0] == '59000100000000000000':
            response = {
                "manifest": {
                    "firmware": {
        ...
        elif path == '/get' and query.get('cfwid', [None])[0] == '59000100000000000000':
            file_path = 'dfu_distr_52840_2.0.zip'
            if os.path.exists(file_path):
                file_size = os.path.getsize(file_path)
                self.send_response(200)
                self.send_header('Content-Type', 'application/gzip')
                self.send_header('Content-Disposition', 'attachment; filename="dfu_distr_52840_2.0.zip"')
                self.send_header('Content-Length', str(file_size))
                self.end_headers()
                with open(file_path, 'rb') as f:
                    self.wfile.write(f.read())
